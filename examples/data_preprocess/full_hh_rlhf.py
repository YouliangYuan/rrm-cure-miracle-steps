...
def generate_sft_dataset(target_hdfs_path_dir, local_dir='~/data/full_hh_rlh/sft'):
    ...
    local_path = os.path.join(local_dir, 'polaris_train_with_think_system_prompt.parquet')

    df.to_parquet(path=local_path)

    if target_hdfs_path_dir is not None:
        hdfs_dir = target_hdfs_path_dir + '/' + 'polaris_train_with_think_system_prompt.parquet'
        makedirs(hdfs_dir)
