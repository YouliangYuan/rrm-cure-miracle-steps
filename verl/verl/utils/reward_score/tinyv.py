...
def tinyv_score(question_str: str, ground_truth: str, model_answer: str, criteria=None, debug=False):
    ...
    if criteria is None:
        ...
        msg = [
            {"role": "user",
             "content": TINYV_PROMPT.replace("{{QUESTION}}", question_str).replace("{{MODEL_ANSWER}}",
                                                                                   model_answer).replace("{{CRITERIA}}",
                                                                                                         criteria)}
        ]
    if debug:
...
def _compute_score(solution_str: str, ground_truth: str, question_str: str, criteria=None, tinyv_setup=None,
                   tinyv_weight=None, debug=False):
    ...
    # Zhangchen: We don't consider format reward for now for consistency with prime
        ...
        if prime_is_correct == False:
            tinyv_reward = tinyv_score(question_str, ground_truth, solution_str, criteria) * tinyv_weight
        else:
            tinyv_reward = 1
        score = tinyv_reward
...
def compute_score(solution_str, ground_truth, extra_info=None, tinyv_setup=None, tinyv_weight=None,
                  format_reward_max=0., tinyv_reward_max=1.):
