        ...
        tasks = [
            asyncio.wait_for(
                loop.run_in_executor(
                    executor,
                    partial(evaluation_func, task, completion, reference, task_extra_info, tinyv_setup, tinyv_weight)
                    # Ensure synchronous
                ),
                timeout=timeout)
        ]
...
async def parallel_compute_score_async(evaluation_func,
                                       ...
                                       num_processes=80,
                                       max_concurrent=500):
    scores = [None] * len(completions)  # 预分配结果列表
    semaphore = asyncio.Semaphore(max_concurrent)  # 限制并发数量

    async def process_single_item(index, completion, reference, task, task_extra_info, executor):
        async with semaphore:  # 使用信号量控制并发
            try:
                result = await single_compute_score(
                    evaluation_func, completion, reference, task,
                    task_extra_info, executor, timeout=300.,
                    tinyv_setup=tinyv_setup, tinyv_weight=tinyv_weight
                )
    ...
    with ProcessPoolExecutor(max_workers=num_processes) as executor:
        # 创建所有任务
        tasks_async = [
            process_single_item(i, completion, reference, task, task_extra_info, executor)
            for i, (completion, reference, task, task_extra_info)
            in enumerate(zip(completions, references, tasks, extra_info))
        ]
...
class PrimeRewardManager:
    ...
    def __init__(
            ...
            tinyv_setup=None, tinyv_weight=None
    ) -> None:
